...
head(WRTDS_allsites)
head(Loadflex_DailySi_allsites)

#rename columns
names(WRTDS_allsites)[names(WRTDS_allsites) == "FluxDay"] = "WRTDS_DailyFlux"
names(Loadflex_DailySi_allsites)[names(Loadflex_DailySi_allsites) == "SiLoad_kg.d"] = "Loadflex_DailyFlux"
names(Loadflex_DailySi_allsites)[names(Loadflex_DailySi_allsites) == "Site"] = "site"

#merge WRTDS and Loadflex results
#keep all results from both models (all=T)
WRTDS_Loadflex_compare = merge(WRTDS_allsites, Loadflex_DailySi_allsites, by=c("site","Date"),all=T)
...
write.csv(WRTDS_Loadflex_compare, file="WRTDS-Loadflex_compare_28Apr21.csv")
...
ggplot(WRTDS_Loadflex_compare, aes(x=Loadflex_DailyFlux, y=WRTDS_DailyFlux)) +
  ...
  ylab("WRTDS model output")+
  xlab("Loadflex model output")+
