...
si_stats<-chem_df %>%
  ...
  dplyr::summarise(mean_si = mean(value_mgL), med_si=median(value_mgL), sd_si=sd(value_mgL))

q_stats<-q_df %>%
  dplyr::group_by(Stream_ID) %>%
  dplyr::summarise(mean_q = mean(Q), med_q=median(Q), sd_q=sd(Q))
...
#merge in DA
ref_table_link<-"https://docs.google.com/spreadsheets/d/11t9YYTzN_T12VAQhHuY5TpVjGS50ymNmKznJK4rKTIU/edit?usp=share_link"
ref_table_folder = drive_get(as_id(ref_table_link))

#download ref table
ref_table<-drive_download(ref_table_folder$drive_resource, overwrite = T)

ref_table<-readxl::read_xlsx("WRTDS_Reference_Table.xlsx")
...
setdiff(cv_tot$Stream_ID, tot$Stream_ID)

write.csv(tot, "AllDrivers_Harmonized.csv")
