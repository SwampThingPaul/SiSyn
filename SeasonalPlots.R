...
setwd("C:/Users/kjankowski/OneDrive - DOI/Documents/Projects/Silica Synthesis/Results/WRTDS")
months = read.csv("WRTDS_MonthlyResults_AllSites_022822.csv")
#months$FNFlux_106kgday=months$FNFlux/(10^6)
...
FNconc$LTER = ifelse(FNconc$site %in% c("ALBION","MARTINELLI","SADDLE STREAM 007"),"NWT",
                     ifelse(FNconc$site %in% c("QS","Q1","Q2","Q3","RI","MPR"), "LUQ", 
                            ifelse(FNconc$site %in% c("ws1","ws2","ws3","ws4","ws5","ws6","ws7","ws8","ws9"), "HBR",
...
FNflux$LTER = ifelse(FNflux$site %in% c("ALBION","MARTINELLI","SADDLE STREAM 007"),"NWT",
                   ifelse(FNflux$site %in% c("QS","Q1","Q2","Q3","RI","MPR"), "LUQ", 
                          ifelse(FNflux$site %in% c("ws1","ws2","ws3","ws4","ws5","ws6","ws7","ws8","ws9"), "HBR",
...
FNyield$LTER = ifelse(FNyield$site %in% c("ALBION","MARTINELLI","SADDLE STREAM 007"),"NWT",
                     ifelse(FNyield$site %in% c("QS","Q1","Q2","Q3","RI","MPR"), "LUQ", 
                            ifelse(FNyield$site %in% c("ws1","ws2","ws3","ws4","ws5","ws6","ws7","ws8","ws9"), "HBR",
...
conc.season=
  ...
  geom_boxplot()+
  facet_wrap(~Biome2, scales="free")+
  theme(axis.text.x=element_text(angle=30))+
  ggtitle("FN concentration change")+
...
yield.season = 
  ...
  theme(axis.text.x=element_text(angle=30))+
  ylab("Sen slope")+xlab("Month")+
  ggtitle("FN yield change")
#ylim(-1000,1000)

plot_grid(conc.season, yield.season,
          ncol = 2, nrow = 1)

setwd("C:/Users/kjankowski/OneDrive - DOI/Documents/Projects/Silica Synthesis/Results/Figures")
ggsave("Figure4_Season_Change_All_Biome.png", height=12, width=8, units="in")
