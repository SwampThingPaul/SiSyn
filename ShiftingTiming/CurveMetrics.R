...
month_agg<-aggregate(Conc_mgL ~ Month+stream, data = month_conc, FUN=mean)

colnames(month_agg)[3]<-"conc"

# sites_unique<-unique(month_agg$stream)
# 
# pdf("NonNormal_SiCurves.pdf")
# 
...
for (i in 1:length(stream_list)) {
  ...
  slope1<-(atan(abs(one_stream$conc_diff[1]))*180)/pi
  
  slope2<-(atan(one_stream$conc_diff[2])*180)/pi
...
for (i in 1:length(stream_list)) {
  ...
  slope1<-(atan(one_stream$conc_diff[1])*180)/pi
  
  slope2<-(atan(abs(one_stream$conc_diff[2]))*180)/pi
...
for (i in 1:length(stream_list)) {
  ...
  #find difference between expected and actual, add to list
  diff[[i]]<-(sum_expected-sum_actual)/sum_expected
