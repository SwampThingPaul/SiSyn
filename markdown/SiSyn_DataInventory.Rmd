...
krr.dat.inv=ddply(krr.dat.melt,c("Sampling.Date","variable","plot.val"),summarise,N.val=N(site))
...
arclter.dat.melt=melt(arclter.dat[,c(idvars,param.vars)],id.vars=idvars)
arclter.dat.melt=subset(arclter.dat.melt,is.na(value)==F)
arclter.dat.melt=merge(arclter.dat.melt,param.list,"variable")
arclter.dat.melt$site=arclter.dat.melt$'Site/Stream.Name'
arclter.dat.melt$variable=as.character(arclter.dat.melt$variable)

arclter.dat.inv=ddply(krr.dat.melt,c("Sampling.Date","variable","plot.val"),summarise,N.val=N(site))
...
lmp.dat.inv=ddply(lmp.dat.melt,c("Sampling.Date","variable","plot.val"),summarise,N.val=N(site))
...
luq.dat.inv=ddply(luq.dat.melt,c("Sampling.Date","variable","plot.val"),summarise,N.val=N(site))
...
pie.dat.inv=ddply(pie.dat.melt,c("Sampling.Date","variable","plot.val"),summarise,N.val=N(site))
...
### Arctic LTER
```{r ALTER,fig.width=7,fig.height=5,fig.align='center',echo=FALSE,warning=FALSE, message=FALSE,fig.cap='Arctic LTER period of record parameter inventory for all sites.'}
...
with(arclter.dat.inv,points(Sampling.Date,plot.val,pch="|",col=adjustcolor("red",0.5)))
axis_fun(1,line=-0.5,xmaj,xmin,format(xmaj,"%m-%Y"))
axis_fun(2,ymaj,ymaj,param.list$variable,cex=0.8)
box(lwd=1)
mtext(side=3,"Arctic LTER",cex=0.8)
...
tmp=cast(arclter.dat.melt,site~variable,value="value",fun.aggregate = function(x)N(x))
...
### Bonanza Creek LTER
```{r BCLTER,fig.width=7,fig.height=5,fig.align='center',echo=FALSE,warning=FALSE, message=FALSE,fig.cap='Bonanza Creek LTER/Caribou-Poker Creeks Research Watershed period of record parameter inventory for all sites.'}
par(family="serif",mar=c(1,3,0.75,0.5),oma=c(2,5,0.5,0.5));
...
### Lamprey River Hydrologic Observatory
```{r LMP,fig.width=7,fig.height=5,fig.align='center',echo=FALSE,warning=FALSE, message=FALSE,fig.cap='Lamprey River Hydrologic Observatory period of record parameter inventory for all sites.'}
par(family="serif",mar=c(1,3,0.75,0.5),oma=c(2,5,0.5,0.5));
...
axis_fun(2,ymaj,ymaj,param.list$variable,cex=0.8)
box(lwd=1)
mtext(side=3,"Lamprey River Hydrologic Observatory",cex=0.8)
mtext(side=1,line=1.75,"Date (Month-Year)")

```

### Plum Island Ecosystems LTER
```{r PIE,fig.width=7,fig.height=5,fig.align='center',echo=FALSE,warning=FALSE, message=FALSE,fig.cap='Plum Island Ecosystems LTER period of record parameter inventory for all sites.'}
par(family="serif",mar=c(1,3,0.75,0.5),oma=c(2,5,0.5,0.5));
...
axis_fun(2,ymaj,ymaj,param.list$variable,cex=0.8)
box(lwd=1)
mtext(side=3,"Plum Island Ecosystems LTER",cex=0.8)
mtext(side=1,line=1.75,"Date (Month-Year)")

```

### Kissimmee River
```{r KRR,fig.width=7,fig.height=5,fig.align='center',echo=FALSE,warning=FALSE, message=FALSE,fig.cap='Kissimmee River period of record parameter inventory for all sites.'}
par(family="serif",mar=c(1,3,0.75,0.5),oma=c(2,5,0.5,0.5));
...
axis_fun(2,ymaj,ymaj,param.list$variable,cex=0.8)
box(lwd=1)
mtext(side=3,"Kissimmee River",cex=0.8)
mtext(side=1,line=1.75,"Date (Month-Year)")

```

### Luquillo LTER
```{r LUQ,fig.width=7,fig.height=5,fig.align='center',echo=FALSE,warning=FALSE, message=FALSE,fig.cap='Luquillo LTER period of record parameter inventory for all sites.'}
par(family="serif",mar=c(1,3,0.75,0.5),oma=c(2,5,0.5,0.5));
ylim.val=c(1,nrow(param.list));ymaj=seq(ylim.val[1],ylim.val[2],1)
plot(xlim.val,0:1,type="n",ylim=ylim.val,xlim=xlim.val,axes=F,ylab=NA,xlab=NA)
abline(v=xmin,h=ymaj,lty=2,col="grey80")
...
axis_fun(2,ymaj,ymaj,param.list$variable,cex=0.8)
box(lwd=1)
mtext(side=3,"Luquillo LTER",cex=0.8)
mtext(side=1,line=1.75,"Date (Month-Year)")
