...
# Authenticate `googledrive` package
pdf_url <- googledrive::as_id("https://drive.google.com/drive/u/0/folders/1UHvNDy7IUFWb91UrYFkbiZTSe2anvUcx")
googledrive::drive_ls(pdf_url)

# Make a folder for exporting outputs to
dir.create(path = file.path(path, "WRTDS Inputs"), showWarnings = F)
dir.create(path = file.path(path, "WRTDS Explore Outputs"), showWarnings = F)
...
for(river in unique(chem_simp$Stream_Element_ID)){
# for(river in "GRO__Yenisey_NOx"){
  ...
  # Fit original model
  egret_estimation <- EGRET::modelEstimation(eList = egret_list, windowS = seasonality,
                                             minNumObs = 50, verbose = F)
  
  # Create file name
  pdf_name <- paste0(out_prefix, seasonality, "_", "Window_Explore.pdf")
  
  # Create PDF report of preliminary graphs
  HERON::egret_report(eList_estim = egret_estimation, eList_series = egret_list_out,
                      out_path = file.path(path, "WRTDS Explore Outputs", pdf_name))
  
  # Upload PDF to GoogleDrive
  googledrive::drive_upload(media = file.path(path, "WRTDS Explore Outputs", pdf_name),
